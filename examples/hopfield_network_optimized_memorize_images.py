...
from networks.hopfield_network import HopfieldNetwork, HopfieldNetworkOptimized
import os
...
images = np.where(images == 0, -1, 1)
images_noisy = images * np.random.choice([1, -1], size=images.shape, p=[0.8, 0.2])
...
for i, (img, img_noisy, img_reconstructed) in enumerate(zip(images, images_noisy, images_reconstructed)):
    ax[i, 0].imshow(img, cmap='gray', aspect='equal'); ax[i, 0].axis('off')
    ax[i, 1].imshow(img_noisy, cmap='gray', aspect='equal'); ax[i, 1].axis('off')
    ax[i, 2].imshow(img_reconstructed, cmap='gray', aspect='equal'); ax[i, 2].axis('off')
